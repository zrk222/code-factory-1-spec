"""Machine-readable SpecLine installation and build provenance."""
...
def _commit(root: Path) -> str | None:
    ...
    if not (source_root / ".git").exists() or not manifest.exists():
        return None
    if 'name = "code-factory-1-spec"' not in manifest.read_text(encoding="utf-8"):
        return None
    try:
...
def provenance() -> dict:
    ...
    origin = "source-tree"
    try:
        distribution = importlib.metadata.distribution("code-factory-1-spec")
    ...
    return {"schema": "factory.provenance.v1", "package": "code-factory-1-spec", "version": __version__,
